...
test_data_image = test_data.test_data[:TEST_DATA_BANTCH]
test_data_image = torch.unsqueeze(test_data_image, 1).type(torch.FloatTensor)
test_data_label = test_data.test_labels[:TEST_DATA_BANTCH]
...
for i in range(EPOCH):
    for step, (bantch_x, bantch_y) in enumerate(dataloader):
        ...
        if step % BANTCH_SIZE == 0:
            ...
            ylim = ax.get_ylim()
            xlim = ax.get_xlim()
            ax.text(xlim[1]/2, ylim[1] - ylim[1] * 0.1,'accuracy rate: {:.2f}'.format(value))
            # legend必须放到plot下面
            ax.legend()
            plt.pause(0.5)
